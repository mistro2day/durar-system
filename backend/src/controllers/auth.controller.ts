...
export const login = async (req: Request, res: Response) => {
  ...
  try {
    if (!email || !password) return res.status(400).json({ message: "البريد وكلمة المرور مطلوبة" });
...
export const forgotPassword = async (req: Request, res: Response) => {
  ...
  const user = await prisma.user.findUnique({ where: { email } });
  if (!user) return res.json({ message: "إن وُجد بريد مطابق سيتم إرسال رابط الاسترجاع" });
...
export const resetPassword = async (req: Request, res: Response) => {
  ...
  if (!token || !newPassword) return res.status(400).json({ message: "بيانات غير صالحة" });
  const row = await prisma.passwordReset.findUnique({ where: { token } });
  if (!row || row.used || row.expiresAt < new Date()) return res.status(400).json({ message: "رمز غير صالح" });
...
export const changePasswordMe = async (req: Request, res: Response) => {
  ...
  if (!id || !oldPassword || !newPassword) return res.status(400).json({ message: "بيانات غير صالحة" });
